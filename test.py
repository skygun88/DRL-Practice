...
def train_minibatch(model: DQN, target_model: DQN, minibatch, optimizer: optim.RMSprop, gamma: float):
    optimizer.zero_grad()
    states = torch.Tensor(np.stack([x[0] for x in minibatch], axis=0)/255)
    ...
    next_states = torch.Tensor(np.stack([x[3] for x in minibatch], axis=0)/255)
...
def main():
    ...
    for epoch in range(max_epoch):
        ...
        while minibatch_cnt < minibatch_train:
            ...
            state = torch.Tensor(next_history/255).unsqueeze(0)
